...
public class DebugGUI : MonoBehaviour
{
    ...
    void DrawGraphs()
    {
        ...
        foreach (GraphContainer graph in graphDictionary.Values)
        {
            ...
            {
                GUI.Box(new Rect((Screen.width / 2) - 250, 100 + graphBlockHeight * graph.group, graphWidth, graphHeight), "", boxStyle);
            }

            graph.Draw(new Rect((Screen.width / 2) - 250, 100 + graphBlockHeight * graph.group, graphWidth, graphHeight));
        }
        ...
        foreach (int group in graphGroupBoxesDrawn)
        {
            ...
            foreach (GraphContainer graph in graphDictionary.Values)
            {
                ...
                if (graph.group == group)
                {
                    ...
                    GUI.Label(new Rect((Screen.width / 2) - 350, groupOrigin +100, minMaxXOffset, graphHeight), labelGuiContent, minMaxTextStyle);
                    // Min
                    labelGuiContent.text = graph.min.ToString("F2");
                    GUI.Label(new Rect((Screen.width / 2) - 350, groupOrigin + graphHeight - textSize.y +100, minMaxXOffset, graphHeight), labelGuiContent, minMaxTextStyle);
                    ...
                    GUI.Label(new Rect((Screen.width / 2) - 350, yOffset +100, xOffset, graphHeight), labelGuiContent);
